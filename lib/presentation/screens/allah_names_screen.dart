...
// Custom clipper for the bottom curved app bar
class BottomAppBarClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    // Create a curved bottom-left corner
    path.quadraticBezierTo(0, size.height, 20, size.height);
    
    // Draw bottom line
    path.lineTo(size.width - 20, size.height);
    
    // Create a curved bottom-right corner
    path.quadraticBezierTo(size.width, size.height, size.width, size.height - 20);
    
    // Draw right line to top-right
...
class _AllahNamesScreenState extends State<AllahNamesScreen> with SingleTickerProviderStateMixin {
  ...
  // Define a flag to check if we can safely use backdrop filter
  final bool _canUseBackdropFilter = !kIsWeb && defaultTargetPlatform != TargetPlatform.android;
      ...
      if (query.isEmpty) {
        ...
        _filteredNames = _allahNames
            .where((name) =>
                ...
                name['english_meaning']!.toLowerCase().contains(query.toLowerCase()))
            .toList();
      }
    });
  }
    ...
    return Scaffold(
      ...
      children: [
        ...
        children: [
          ...
          AnimatedContainer(
          duration: const Duration(milliseconds: 300),
          height: _isSearching ? screenHeight * 0.08 : 0,
          ...
          // Advertisement at the bottom - fixed to avoid backdrop filter
          _buildSafeAdvertContainer(screenWidth, screenHeight),
        ],
        ),
      ],
      ),
  ...
  // Safe version of app bar that avoids backdrop filter issues
  Widget _buildSafeAppBar({required String title, required double screenWidth}) {
    if (_canUseBackdropFilter) {
      return ClipRRect(
        child: BackdropFilter(
          filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10),
          child: AppBar(
      ...
      // Fallback version without backdrop filter
      return AppBar(
        title: Text(
          title,
          style: TextStyle(
  ...
  // Safe version of advertisement container
  Widget _buildSafeAdvertContainer(double screenWidth, double screenHeight) {
    if (_canUseBackdropFilter) {
      return ClipRect(
        child: BackdropFilter(
    ...
    return Hero(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          borderRadius: BorderRadius.circular(20),
          splashColor: Color(0xFF00BFA5).withOpacity(0.1),
          highlightColor: Color(0xFF00BFA5).withOpacity(0.05),
          child: Container(
            decoration: BoxDecoration(
            ...
            child: Stack(
              children: [
                ...
                // Content - Centered everything
                Center(
                  child: Padding(
                    padding: EdgeInsets.symmetric(horizontal: screenWidth * 0.03, vertical: screenWidth * 0.04),
                    child: Column(
                ...
                Row(
                  ...
                  children: [
                    ...
                    _buildDetailActionButton(
                      ...
                      label: 'Copy',
                      onPressed: () {
                        // Copy name details to clipboard
                        final textToCopy = '$englishName ($arabicName)\n$banglaName - $meaning\nMeaning: $englishMeaning';
